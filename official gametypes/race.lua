...
alias all_mongooses = 5
alias all_warthogs  = 6
alias all_banshees  = 7
alias all_ghosts    = 8
...
on double host migration: do
   for each object with label all_mongooses do
      current_object.delete()
   end
   for each object with all_warthogs do
      current_object.delete()
   end
   for each object with all_ghosts do
      current_object.delete()
   end
   for each object with all_banshees do
      current_object.delete()
   end
   for each object with label "race_flag" do
      current_object.checkpoint_id = current_object.spawn_sequence
...
for each player do -- create player vehicle when there is none
   ...
   if current_player.vehicle == no_object then 
      ...
      if opt_vehicle == 2 then 
         new_vehicle = current_player.biped.place_at_me(warthog, none, none, 0, 0, 0, none)
      end
      if opt_vehicle == 3 then 
         new_vehicle = current_player.biped.place_at_me(ghost, none, none, 0, 0, 0, none)
      end
      if opt_vehicle == 4 then 
         new_vehicle = current_player.biped.place_at_me(banshee, none, none, 0, 0, 0, none)
      end
...
for each player do -- game start trigger (round card and some Race-specific state preparations)
   ...
   script_widget[1].set_text("next gate: %n  (%nm)", hud_player.next_checkpoint_number, hud_player.distance_to_next_checkpoint)
   script_widget[2].set_text("lap time: %s", hud_player.current_lap_time)
...
do -- update Distance Traveled stat
   distance_traveled_update_timer.set_rate(-100%)
...
for each player do
   ...
   if current_player.is_in_vehicle == 1 then 
      --
...
for each player do -- track whether the player is on foot, and reset them if they're on foot for too long
   ...
   if global.object[1] == no_object then
      script_widget[0].set_visibility(current_player, false) -- hide the Race UI when the player is on foot
      script_widget[2].set_visibility(current_player, false)
      script_widget[1].set_visibility(current_player, false)
